#!/usr/bin/env python3
"""
Phase 4: EXECUTIE REALA - Stergere simetrica a 1202 perechi mismatched (FIXED)
"""
...
print("\n" + "="*120)
print("PHASE 4: EXECUTIE REALA - STERGERE SIMETRICA (FIXED)")
print("="*120)
...
for idx, pair in enumerate(mismatched, 1):
    ...
    # Extract filename from before path
    filename = os.path.basename(before_path)  # e.g., "3FO4IDU7VMMJ_538631_empty_1.jpg"
    ...
    if os.path.exists(before_path):
        try:
            ...
            print(f"Failed to delete before: {before_path}: {e}")
    ...
    if os.path.exists(after_path):
        try:
            ...
            print(f"Failed to delete after: {after_path}: {e}")
            deleted_stats['failed'] += 1
    
    # Sterge MASK - masks folder contine aceleasi fisiere jpg
    dataset_path = os.path.join(BASE_DATA_DIR, dataset)
    mask_path = os.path.join(dataset_path, 'masks', filename)
    if os.path.exists(mask_path):
        try:
            os.remove(mask_path)
            deleted_stats['masks'] += 1
        except Exception as e:
            print(f"Failed to delete mask: {mask_path}: {e}")
            deleted_stats['failed'] += 1
    
    # Sterge MASK_CLEAN (train only, contine aceleasi fisiere jpg)
    if dataset == 'train':
        mask_clean_path = os.path.join(dataset_path, 'masks_clean', filename)
        if os.path.exists(mask_clean_path):
            try:
                os.remove(mask_clean_path)
                deleted_stats['masks_clean'] += 1
            except Exception as e:
                print(f"Failed to delete mask_clean: {mask_clean_path}: {e}")
                deleted_stats['failed'] += 1
...
print(f"\n" + "-"*120)
print(f"TOTAL PERECHI RAMASE: {total_remaining}")
print(f"TOTAL PERECHI ELIMINATE: {len(mismatched)}")
print(f"RATA REDUCERE: {(len(mismatched)/1616)*100:.1f}%")

print(f"\n" + "="*120)
if all_synced and deleted_stats['failed'] == 0:
    print("✓ SUCCESS: Toti fisierul au fost stersi simetric si perfect sincronizati!")
else:
    print("✗ WARNING: Exista inconsistente sau erori!")
print("="*120 + "\n")
